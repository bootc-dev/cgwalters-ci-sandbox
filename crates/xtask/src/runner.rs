...
fn create_run_dir(root: &Utf8PathBuf) -> Result<Utf8PathBuf> {
    ...
    // Consumers such as bcvk require TMT_LOG_DIR to be absolute.  Resolve the
    // default relative root before exporting paths derived from it.
    let root = fs::canonicalize(root).with_context(|| format!("Resolving log root {root}"))?;
    let root = Utf8PathBuf::from_path_buf(root)
        .map_err(|root| anyhow::anyhow!("Log root is not valid UTF-8: {}", root.display()))?;
...
mod tests {
    ...
    #[test]
    fn resolves_relative_log_roots_to_absolute_paths() {
        const CHILD_ENV: &str = "BOOTC_RUNNER_RELATIVE_ROOT_CHILD";

        if std::env::var_os(CHILD_ENV).is_some() {
            let run_dir = create_run_dir(&Utf8PathBuf::from("logs")).unwrap();
            let cwd = Utf8PathBuf::from_path_buf(std::env::current_dir().unwrap()).unwrap();
            assert!(run_dir.is_absolute());
            assert!(run_dir.starts_with(&cwd));
            return;
        }

        // Run the relative-path case in a writable child working directory.
        // The parent may be running from a read-only source checkout, and tests
        // run concurrently so changing this process's current directory is unsafe.
        let temp = tempfile::tempdir().unwrap();
        let status = Command::new(std::env::current_exe().unwrap())
            .args([
                "--exact",
                "runner::tests::resolves_relative_log_roots_to_absolute_paths",
            ])
            .env(CHILD_ENV, "1")
