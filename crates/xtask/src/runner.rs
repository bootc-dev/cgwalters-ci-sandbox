...
mod tests {
    ...
    fn resolves_relative_log_roots_to_absolute_paths() {
        ...
        let temp = tempfile::Builder::new()
            .prefix(".runner-relative-")
            .tempdir_in(&cwd)
            .unwrap();
        let relative = temp.path().strip_prefix(&cwd).unwrap().join("logs");
        let relative = Utf8PathBuf::from_path_buf(relative).unwrap();

        let run_dir = create_run_dir(&relative).unwrap();
        assert!(run_dir.is_absolute());
        assert!(run_dir.starts_with(Utf8PathBuf::from_path_buf(cwd).unwrap()));
    }
