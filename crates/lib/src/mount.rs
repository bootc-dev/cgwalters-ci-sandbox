...
fn validate_record(record: &MountRecord) -> Result<()> {
    ...
    validate_mount_topology(expected, actual_paths, allowed_missing)?;
...
fn validate_mount_topology(
    mut expected: std::collections::BTreeSet<PathBuf>,
    actual: std::collections::BTreeSet<PathBuf>,
    allowed_missing: Option<&PathBuf>,
) -> Result<()> {
    if actual == expected {
        return Ok(());
    }
    if let Some(path) = allowed_missing {
        let mut after_pending_unmount = expected.clone();
        after_pending_unmount.remove(path);
        if actual == after_pending_unmount {
            return Ok(());
        }
        if !actual.contains(path) {
            expected = after_pending_unmount;
        }
    }
    let missing: std::collections::BTreeSet<_> = expected.difference(&actual).collect();
    let unexpected: std::collections::BTreeSet<_> = actual.difference(&expected).collect();
    ensure!(
        missing.is_empty() && unexpected.is_empty(),
        "mount topology changed; refusing unmount; missing: {missing:?}; unexpected: {unexpected:?}"
    );
    Ok(())
}
...
pub(crate) async fn mount(mut opts: MountOpts) -> Result<()> {
    ...
    // findmnt reports canonical mount targets.  Record the same spelling so
    // aliases such as Fedora's /mnt -> /var/mnt do not look like topology
...
pub(crate) async fn unmount(opts: UnmountOpts) -> Result<()> {
    ...
    let target = canonicalize_utf8_path(&opts.target, "mount target")?;
...
mod tests {
    ...
    use super::{
        MountIdentity, MountRecord, canonicalize_utf8_path, collect_mount_targets,
        ostree_state_path, pending_unmount_recovered, validate_mount_target,
        validate_mount_topology, validate_record_shape, validate_relative_mount_path,
    };
    ...
    #[test]
    fn canonicalizes_mount_target_aliases() {
        ...
        let aliased_target = Utf8PathBuf::from_path_buf(alias.join("target")).unwrap();

        assert_eq!(
            canonicalize_utf8_path(&aliased_target, "mount target").unwrap(),
    ...
    #[test]
    fn topology_diagnostic_reports_missing_and_unexpected_paths() {
        let expected: std::collections::BTreeSet<PathBuf> =
            ["/target", "/target/etc", "/target/var"]
                .into_iter()
                .map(PathBuf::from)
                .collect();
        let actual = ["/target", "/target/etc", "/target/tmp"]
            .into_iter()
            .map(PathBuf::from)
            .collect();

        let error = validate_mount_topology(expected, actual, None).unwrap_err();
        assert_eq!(
            error.to_string(),
            "mount topology changed; refusing unmount; missing: {\"/target/var\"}; unexpected: {\"/target/tmp\"}"
        );
        ...
        let expected: std::collections::BTreeSet<PathBuf> =
            ["/target", "/target/etc", "/target/var"]
                ...
                .map(PathBuf::from)
                .collect();
        let pending = PathBuf::from("/target/var");
        for actual in [
            ["/target", "/target/etc", "/target/var"].as_slice(),
            ["/target", "/target/etc"].as_slice(),
        ] {
            let actual = actual.iter().map(PathBuf::from).collect();
            validate_mount_topology(expected.clone(), actual, Some(&pending)).unwrap();
        }
    }
